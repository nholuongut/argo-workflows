...
func TestSemaphoreTmplLevel(t *testing.T) {
	...
	t.Run("TmplLevelAcquireAndRelease", func(t *testing.T) {
		...
		wf.Name = "one"
		wf, err := controller.wfclientset.nholuongutV1alpha1().Workflows(wf.Namespace).Create(ctx, wf, metav1.CreateOptions{})
		assert.NoError(t, err)
		...
		wf_Two := wf.DeepCopy()
		wf_Two.Name = "two"
		wf_Two, err = controller.wfclientset.nholuongutV1alpha1().Workflows(wf.Namespace).Create(ctx, wf_Two, metav1.CreateOptions{})
		assert.NoError(t, err)
...
func TestSynchronizationWithStep(t *testing.T) {
	...
	t.Run("StepWithSychronization", func(t *testing.T) {
		...
		wf, err := controller.wfclientset.nholuongutV1alpha1().Workflows("default").Create(ctx, wf, metav1.CreateOptions{})
		assert.NoError(err)
		woc := newWorkflowOperationCtx(wf, controller)
		woc.operate(ctx)
		...
		wf1.Name = "step2"
		wf1, err = controller.wfclientset.nholuongutV1alpha1().Workflows("default").Create(ctx, wf1, metav1.CreateOptions{})
		assert.NoError(err)
...
const wfWithStepRetry = `apiVersion: nholuongut.io/v1alpha1
kind: Workflow
metadata:
