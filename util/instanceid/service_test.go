...
func TestWith(t *testing.T) {
	t.Run("Empty", func(t *testing.T) {
		...
		assert.Equal(t, "!workflows.nholuongut.io/controller-instanceid", opts.LabelSelector)
	})
	t.Run("EmptyExistingSelector", func(t *testing.T) {
		opts := &metav1.ListOptions{LabelSelector: "foo"}
		NewService("").With(opts)
		assert.Equal(t, "foo,!workflows.nholuongut.io/controller-instanceid", opts.LabelSelector)
	})
	t.Run("ExistingSelector", func(t *testing.T) {
		opts := &metav1.ListOptions{LabelSelector: "foo"}
		NewService("foo").With(opts)
		assert.Equal(t, "foo,workflows.nholuongut.io/controller-instanceid=foo", opts.LabelSelector)
	})
}
