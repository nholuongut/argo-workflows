...
import (
	...
	argofile "github.com/nholuongut/pkg/file"
	...
	artifact "github.com/nholuongut/argo-workflows/v3/workflow/artifacts"
	artifactcommon "github.com/nholuongut/argo-workflows/v3/workflow/artifacts/common"
...
func errWithHelp(err error) error {
	return fmt.Errorf("unable to get pods, you can check https://nholuongut.github.io/argo-workflows/faq/: %w", err)
}
		...
		if we.isBaseImagePath(param.ValueFrom.Path) {
			...
			if executorType == common.ContainerRuntimeExecutorK8sAPI || executorType == common.ContainerRuntimeExecutorKubelet {
				log.Infof("Copying output parameter %s from base image layer %s is not supported for k8sapi and kubelet executors. "+
					"Consider using an emptyDir volume: https://nholuongut.github.io/argo-workflows/empty-dir/.", param.Name, param.ValueFrom.Path)
				continue
			}
