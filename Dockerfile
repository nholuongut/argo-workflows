...
WORKDIR /go/src/github.com/nholuongut/argo-workflows
COPY go.mod .
COPY go.sum .
RUN go mod download
...
COPY --from=argoexec-build /go/src/github.com/nholuongut/argo-workflows/dist/argoexec /usr/local/bin/
ENTRYPOINT [ "argoexec" ]
...
COPY --chown=8737 --from=workflow-controller-build /go/src/github.com/nholuongut/argo-workflows/dist/workflow-controller /bin/
...
COPY --from=argocli-build /go/src/github.com/nholuongut/argo-workflows/dist/argo /bin/
