...
func TestNewOperation(t *testing.T) {
	...
	// assert
	list, err := client.nholuongutV1alpha1().Workflows("my-ns").List(ctx, metav1.ListOptions{})
	...
	assert.Equal(t, "Warning WorkflowEventBindingError failed to dispatch event: failed to get workflow template: workflowtemplates.nholuongut.io \"not-found\" not found", <-recorder.Events)
