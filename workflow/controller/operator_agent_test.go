...
var httpwf = `apiVersion: nholuongut.io/v1alpha1
kind: Workflow
metadata:
  name: hello-world
  namespace: default
...
var taskSet = `apiVersion: nholuongut.io/v1alpha1
...
metadata:
  ...
  selfLink: /apis/nholuongut.io/v1alpha1/namespaces/default/workflowtasksets/hello-world
...
func TestHTTPTemplate(t *testing.T) {
	...
	t.Run("ExecuteHTTPTemplate", func(t *testing.T) {
		...
		// tss, err :=controller.wfclientset.nholuongutV1alpha1().WorkflowTaskSets(wf.Namespace).List(ctx, metav1.ListOptions{})
		ts, err := controller.wfclientset.nholuongutV1alpha1().WorkflowTaskSets(wf.Namespace).Get(ctx, "hello-world", metav1.GetOptions{})
		assert.NoError(t, err)
		assert.NotNil(t, ts)
