...
export const buildGraph = (eventSources: EventSource[], sensors: Sensor[], workflows: Workflow[], flow: {[p: string]: {count: number; timeout?: any}}, expanded: boolean) => {
    ...
    (workflows || []).forEach(workflow => {
        const sensorName = workflow.metadata.labels['events.nholuongut.io/sensor'];
        const triggerName = workflow.metadata.labels['events.nholuongut.io/trigger'];
    ...
    Object.entries(workflowGroups).forEach(([triggerId, items]) => {
        items.forEach((workflow, i) => {
            ...
            if (expanded || i < numNodesToHide || items.length <= numNodesToHide + 1) {
                ...
                const phase = workflow.metadata.labels['workflows.nholuongut.io/phase'];
