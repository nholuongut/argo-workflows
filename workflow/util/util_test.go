...
import (
	...
	argofake "github.com/nholuongut/argo-workflows/v3/pkg/client/clientset/versioned/fake"
...
metadata:
  ...
  selfLink: /apis/nholuongut.io/v1alpha1/namespaces/argo/workflows/suspend
...
func TestResumeWorkflowByNodeName(t *testing.T) {
	wfIf := argofake.NewSimpleClientset().nholuongutV1alpha1().Workflows("")
...
func TestUpdateSuspendedNode(t *testing.T) {
	...
	if assert.NoError(t, err) {
		...
		assert.EqualError(t, err, "workflows.nholuongut.io \"does-not-exist\" not found")
...
metadata:
  ...
  selfLink: /apis/nholuongut.io/v1alpha1/namespaces/argo/workflows/steps-9fkqc
...
func TestRetryWorkflow(t *testing.T) {
	...
	wfClient := argofake.NewSimpleClientset().nholuongutV1alpha1().Workflows("my-ns")
...
func TestFromUnstructuredObj(t *testing.T) {
	...
	wfv1.MustUnmarshal([]byte(`apiVersion: nholuongut.io/v1alpha1
kind: CronWorkflow
metadata:
