...
import (
	...
	"github.com/nholuongut/argo-workflows/v3/util/sorting"
	"github.com/nholuongut/argo-workflows/v3/util/template"
	"github.com/nholuongut/argo-workflows/v3/workflow/artifacts/hdfs"
...
func (ctx *templateValidationCtx) validateBaseImageOutputs(tmpl *wfv1.Template) error {
	...
	case common.ContainerRuntimeExecutorPNS:
		...
		errMsg := "pns executor does not support outputs from base image layer with volume mounts. Use an emptyDir: https://nholuongut.github.io/argo-workflows/empty-dir/"
	...
	case common.ContainerRuntimeExecutorK8sAPI, common.ContainerRuntimeExecutorKubelet:
		...
		errMsg := fmt.Sprintf("%s executor does not support outputs from base image layer.  Use an emptyDir: https://nholuongut.github.io/argo-workflows/empty-dir/", ctx.ContainerRuntimeExecutor)
		for _, out := range tmpl.Outputs.Artifacts {
			if common.FindOverlappingVolume(tmpl, out.Path) == nil {
				return errors.Errorf(errors.CodeBadRequest, "templates.%s.outputs.artifacts.%s: %s", tmpl.Name, out.Name, errMsg)
			}
