...
func TestHTTPArtifactDriver_Load(t *testing.T) {
	...
	a := &wfv1.HTTPArtifact{
		URL: "https://github.com/nholuongut/argo-workflows",
	}
	...
	t.Run("FoundWithRequestHeaders", func(t *testing.T) {
		...
		curl := "curl -fsS -L --create-dirs --proto-default https -o /tmp/found-with-request-headers https://github.com/nholuongut/argo-workflows -H Accept: application/json -H Authorization: Bearer foo-bar"
		assert.Regexp(t, regexp.MustCompile(curl), output)
	})
	t.Run("NotFound", func(t *testing.T) {
		err := driver.Load(&wfv1.Artifact{
			ArtifactLocation: wfv1.ArtifactLocation{
				HTTP: &wfv1.HTTPArtifact{URL: "https://github.com/nholuongut/argo-workflows/not-found"},
			},
		}, "/tmp/not-found")
