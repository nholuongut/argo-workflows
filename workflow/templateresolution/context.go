...
func (n *NullClusterWorkflowTemplateGetter) Get(name string) (*wfv1.ClusterWorkflowTemplate, error) {
	return nil, errors.Errorf("", "invalid spec: clusterworkflowtemplates.nholuongut.io `%s` is "+
		"forbidden: User cannot get resource 'clusterworkflowtemplates' in API group nholuongut.io at the cluster scope", name)
}
