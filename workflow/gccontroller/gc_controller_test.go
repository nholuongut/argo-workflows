...
metadata:
  ...
  selfLink: /apis/nholuongut.io/v1alpha1/namespaces/default/workflows/hello-world-nrgbf
...
metadata:
  ...
  selfLink: /apis/nholuongut.io/v1alpha1/namespaces/argo/workflows/workflow-template-hello-world-k4d26
...
metadata:
  ...
  selfLink: /apis/nholuongut.io/v1alpha1/namespaces/argo/workflows/workflow-template-hello-world-jdkdw
...
func TestTTLStrategySucceded(t *testing.T) {
	...
	_, err = controller.wfclientset.nholuongutV1alpha1().Workflows("default").Create(ctx, wf2, metav1.CreateOptions{})
	assert.NoError(t, err)
	enqueueWF(controller, un)
	...
	_, err = controller.wfclientset.nholuongutV1alpha1().Workflows("default").Create(ctx, wf3, metav1.CreateOptions{})
	assert.NoError(t, err)
	enqueueWF(controller, un)
	controller.processNextWorkItem(ctx)
	assert.Equal(t, 1, controller.workqueue.Len())
