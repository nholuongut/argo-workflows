...
import (
	...
	"github.com/nholuongut/argo-workflows/v3/util/kubeconfig"
...
func (s *E2ESuite) SetupSuite() {
	...
	s.wfClient = versioned.NewForConfigOrDie(s.RestConfig).nholuongutV1alpha1().Workflows(Namespace)
	s.wfebClient = versioned.NewForConfigOrDie(s.RestConfig).nholuongutV1alpha1().WorkflowEventBindings(Namespace)
	s.wfTemplateClient = versioned.NewForConfigOrDie(s.RestConfig).nholuongutV1alpha1().WorkflowTemplates(Namespace)
	s.cronClient = versioned.NewForConfigOrDie(s.RestConfig).nholuongutV1alpha1().CronWorkflows(Namespace)
	s.Persistence = newPersistence(s.KubeClient, s.Config)
	s.hydrator = hydrator.New(s.Persistence.offloadNodeStatusRepo)
	s.cwfTemplateClient = versioned.NewForConfigOrDie(s.RestConfig).nholuongutV1alpha1().ClusterWorkflowTemplates()
}
