...
func cleanCRD(filename string) {
	...
	switch name {
	case "cronworkflows.nholuongut.io":
	...
	case "clusterworkflowtemplates.nholuongut.io", "workflows.nholuongut.io", "workflowtemplates.nholuongut.io":
		properties := schema["properties"].(obj)["spec"].(obj)["properties"].(obj)["templates"].(obj)["items"].(obj)["properties"]
		properties.(obj)["container"].(obj)["required"] = []string{"image"}
		properties.(obj)["script"].(obj)["required"] = []string{"image", "source"}
	case "workfloweventbindings.nholuongut.io":
		// noop
	case "workflowtasksets.nholuongut.io":
		// noop
	default:
		panic(name)
	}
