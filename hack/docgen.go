...
func getExamples(examples Set, summary string) string {
	...
	for _, example := range sortedSetKeys(examples) {
		...
		out += fmt.Sprintf(listElement, link(fmt.Sprintf("`%s`", name), "https://github.com/nholuongut/argo-workflows/blob/master/"+example))
	}
	out += dropdownCloser
	return out
}
...
func getObjectType(field map[string]interface{}, addToQueue func(string)) string {
	...
	if objTypeRaw == "array" {
		if ref, ok := field["items"].(map[string]interface{})["$ref"]; ok {
			...
			if refString == "io.nholuongut.workflow.v1alpha1.ParallelSteps" {
...
func NewDocGeneratorContext() *DocGeneratorContext {
	return &DocGeneratorContext{
		doneFields: make(Set),
		queue: []string{
			"io.nholuongut.workflow.v1alpha1.Workflow", "io.nholuongut.workflow.v1alpha1.CronWorkflow",
			"io.nholuongut.workflow.v1alpha1.WorkflowTemplate",
		},
		external: []string{},
...
func (c *DocGeneratorContext) addToQueue(ref, jsonFieldName string) {
	if ref == "io.nholuongut.workflow.v1alpha1.ParallelSteps" {
		ref = "io.nholuongut.workflow.v1alpha1.WorkflowStep"
	}
	if _, ok := c.doneFields[ref]; !ok {
		c.doneFields[ref] = true
		c.jsonName[ref] = jsonFieldName
		if strings.Contains(ref, "io.nholuongut.workflow") {
