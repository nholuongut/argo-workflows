...
import (
	...
	"github.com/nholuongut/argo-workflows/v3/pkg/client/clientset/versioned/fake"
...
var scheduledWf = `
  ...
  metadata:
    ...
    selfLink: /apis/nholuongut.io/v1alpha1/namespaces/argo/cronworkflows/hello-world
...
func TestCronWorkflowConditionSubmissionError(t *testing.T) {
	...
	woc := &cronWfOperationCtx{
		wfClientset:       cs,
		wfClient:          cs.nholuongutV1alpha1().Workflows(""),
		cronWfIf:          cs.nholuongutV1alpha1().CronWorkflows(""),
		cronWf:            &cronWf,
		log:               logrus.WithFields(logrus.Fields{}),
		metrics:           testMetrics,
...
func TestScheduleTimeParam(t *testing.T) {
	...
	woc.Run()
	wsl, err := cs.nholuongutV1alpha1().Workflows("").List(context.Background(), v1.ListOptions{})
	assert.NoError(t, err)
...
const lastUsedSchedule = `apiVersion: nholuongut.io/v1alpha1
kind: CronWorkflow
metadata:
  name: test
spec:
