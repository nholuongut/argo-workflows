...
import (
	...
	"github.com/nholuongut/argo-workflows/v3/persist/sqldb"
	...
	"github.com/nholuongut/argo-workflows/v3/pkg/client/clientset/versioned"
	"github.com/nholuongut/argo-workflows/v3/server/auth"
	argoutil "github.com/nholuongut/argo-workflows/v3/util"
	"github.com/nholuongut/argo-workflows/v3/util/fields"
	"github.com/nholuongut/argo-workflows/v3/util/instanceid"
	"github.com/nholuongut/argo-workflows/v3/util/logs"
	"github.com/nholuongut/argo-workflows/v3/workflow/common"
	"github.com/nholuongut/argo-workflows/v3/workflow/creator"
	"github.com/nholuongut/argo-workflows/v3/workflow/hydrator"
	"github.com/nholuongut/argo-workflows/v3/workflow/templateresolution"
	"github.com/nholuongut/argo-workflows/v3/workflow/util"
	"github.com/nholuongut/argo-workflows/v3/workflow/validate"
)
...
func (s *workflowServer) CreateWorkflow(ctx context.Context, req *workflowpkg.WorkflowCreateRequest) (*wfv1.Workflow, error) {
	...
	wftmplGetter := templateresolution.WrapWorkflowTemplateInterface(wfClient.nholuongutV1alpha1().WorkflowTemplates(req.Namespace))
	cwftmplGetter := templateresolution.WrapClusterWorkflowTemplateInterface(wfClient.nholuongutV1alpha1().ClusterWorkflowTemplates())
	...
	wf, err := wfClient.nholuongutV1alpha1().Workflows(req.Namespace).Create(ctx, req.Workflow, metav1.CreateOptions{})
	if err != nil {
...
func (s *workflowServer) ListWorkflows(ctx context.Context, req *workflowpkg.WorkflowListRequest) (*wfv1.WorkflowList, error) {
	...
	wfList, err := wfClient.nholuongutV1alpha1().Workflows(req.Namespace).List(ctx, *listOption)
	if err != nil {
		return nil, err
	}
	cleaner := fields.NewCleaner(req.Fields)
...
func (s *workflowServer) WatchWorkflows(req *workflowpkg.WatchWorkflowsRequest, ws workflowpkg.WorkflowService_WatchWorkflowsServer) error {
	...
	wfIf := wfClient.nholuongutV1alpha1().Workflows(req.Namespace)
...
func (s *workflowServer) DeleteWorkflow(ctx context.Context, req *workflowpkg.WorkflowDeleteRequest) (*workflowpkg.WorkflowDeleteResponse, error) {
	...
	err = auth.GetWfClient(ctx).nholuongutV1alpha1().Workflows(wf.Namespace).Delete(ctx, wf.Name, metav1.DeleteOptions{PropagationPolicy: argoutil.GetDeletePropagation()})
	if err != nil {
		return nil, err
	}
...
func (s *workflowServer) RetryWorkflow(ctx context.Context, req *workflowpkg.WorkflowRetryRequest) (*wfv1.Workflow, error) {
	...
	wf, err = util.RetryWorkflow(ctx, kubeClient, s.hydrator, wfClient.nholuongutV1alpha1().Workflows(req.Namespace), wf.Name, req.RestartSuccessful, req.NodeFieldSelector)
	if err != nil {
		return nil, err
	}
	return wf, nil
...
func (s *workflowServer) ResubmitWorkflow(ctx context.Context, req *workflowpkg.WorkflowResubmitRequest) (*wfv1.Workflow, error) {
	...
	created, err := util.SubmitWorkflow(ctx, wfClient.nholuongutV1alpha1().Workflows(req.Namespace), wfClient, req.Namespace, newWF, &wfv1.SubmitOpts{})
	if err != nil {
		return nil, err
	}
	return created, nil
...
func (s *workflowServer) ResumeWorkflow(ctx context.Context, req *workflowpkg.WorkflowResumeRequest) (*wfv1.Workflow, error) {
	...
	err = util.ResumeWorkflow(ctx, wfClient.nholuongutV1alpha1().Workflows(req.Namespace), s.hydrator, wf.Name, req.NodeFieldSelector)
	if err != nil {
		log.Warnf("Failed to resume %s: %+v", wf.Name, err)
		return nil, err
	}

	wf, err = wfClient.nholuongutV1alpha1().Workflows(req.Namespace).Get(ctx, wf.Name, metav1.GetOptions{})
	if err != nil {
		return nil, err
	}
...
func (s *workflowServer) SuspendWorkflow(ctx context.Context, req *workflowpkg.WorkflowSuspendRequest) (*wfv1.Workflow, error) {
	...
	err = util.SuspendWorkflow(ctx, wfClient.nholuongutV1alpha1().Workflows(wf.Namespace), wf.Name)
	if err != nil {
		return nil, err
	}
...
func (s *workflowServer) TerminateWorkflow(ctx context.Context, req *workflowpkg.WorkflowTerminateRequest) (*wfv1.Workflow, error) {
	...
	err = util.TerminateWorkflow(ctx, wfClient.nholuongutV1alpha1().Workflows(req.Namespace), wf.Name)
	if err != nil {
		return nil, err
	}
...
func (s *workflowServer) StopWorkflow(ctx context.Context, req *workflowpkg.WorkflowStopRequest) (*wfv1.Workflow, error) {
	...
	err = util.StopWorkflow(ctx, wfClient.nholuongutV1alpha1().Workflows(req.Namespace), s.hydrator, wf.Name, req.NodeFieldSelector, req.Message)
	if err != nil {
		return nil, err
	}
...
func (s *workflowServer) SetWorkflow(ctx context.Context, req *workflowpkg.WorkflowSetRequest) (*wfv1.Workflow, error) {
	...
	err = util.SetWorkflow(ctx, wfClient.nholuongutV1alpha1().Workflows(req.Namespace), s.hydrator, wf.Name, req.NodeFieldSelector, operation)
	if err != nil {
		return nil, err
	}
...
func (s *workflowServer) getWorkflow(ctx context.Context, wfClient versioned.Interface, namespace string, name string, options metav1.GetOptions) (*wfv1.Workflow, error) {
	...
	wf, err := wfClient.nholuongutV1alpha1().Workflows(namespace).Get(ctx, name, options)
	if err != nil {
		return nil, err
	}
	return wf, nil
...
func getLatestWorkflow(ctx context.Context, wfClient versioned.Interface, namespace string) (*wfv1.Workflow, error) {
	wfList, err := wfClient.nholuongutV1alpha1().Workflows(namespace).List(ctx, metav1.ListOptions{})
	if err != nil {
		return nil, err
	}
...
func (s *workflowServer) SubmitWorkflow(ctx context.Context, req *workflowpkg.WorkflowSubmitRequest) (*wfv1.Workflow, error) {
	...
	case workflow.CronWorkflowKind, workflow.CronWorkflowSingular, workflow.CronWorkflowPlural, workflow.CronWorkflowShortName:
		cronWf, err := wfClient.nholuongutV1alpha1().CronWorkflows(req.Namespace).Get(ctx, req.ResourceName, metav1.GetOptions{})
		if err != nil {
			return nil, err
		}
	...
	case workflow.WorkflowTemplateKind, workflow.WorkflowTemplateSingular, workflow.WorkflowTemplatePlural, workflow.WorkflowTemplateShortName:
		wfTmpl, err := wfClient.nholuongutV1alpha1().WorkflowTemplates(req.Namespace).Get(ctx, req.ResourceName, metav1.GetOptions{})
		if err != nil {
			return nil, err
		}
	...
	case workflow.ClusterWorkflowTemplateKind, workflow.ClusterWorkflowTemplateSingular, workflow.ClusterWorkflowTemplatePlural, workflow.ClusterWorkflowTemplateShortName:
		cwfTmpl, err := wfClient.nholuongutV1alpha1().ClusterWorkflowTemplates().Get(ctx, req.ResourceName, metav1.GetOptions{})
		if err != nil {
			return nil, err
		}
	...
	return wfClient.nholuongutV1alpha1().Workflows(req.Namespace).Create(ctx, wf, metav1.CreateOptions{})
}
