...
override LDFLAGS += \
  -X github.com/nholuongut/argo-workflows/v3.version=$(VERSION) \
  -X github.com/nholuongut/argo-workflows/v3.buildDate=${BUILD_DATE} \
  -X github.com/nholuongut/argo-workflows/v3.gitCommit=${GIT_COMMIT} \
  -X github.com/nholuongut/argo-workflows/v3.gitTreeState=${GIT_TREE_STATE}

ifneq ($(GIT_TAG),)
override LDFLAGS += -X github.com/nholuongut/argo-workflows/v3.gitTag=${GIT_TAG}
endif

ARGOEXEC_PKGS    := $(shell echo cmd/argoexec            && go list -f '{{ join .Deps "\n" }}' ./cmd/argoexec/            | grep 'nholuongut/argo-workflows/v3/' | cut -c 39-)
CLI_PKGS         := $(shell echo cmd/argo                && go list -f '{{ join .Deps "\n" }}' ./cmd/argo/                | grep 'nholuongut/argo-workflows/v3/' | cut -c 39-)
CONTROLLER_PKGS  := $(shell echo cmd/workflow-controller && go list -f '{{ join .Deps "\n" }}' ./cmd/workflow-controller/ | grep 'nholuongut/argo-workflows/v3/' | cut -c 39-)
...
CRDS := $(shell find manifests/base/crds -type f -name 'nholuongut.io_*.yaml')
...
define protoc
    ...
    protoc \
      ...
      $(1)
     perl -i -pe 's|nholuongut/argo-workflows/|nholuongut/argo-workflows/v3/|g' `echo "$(1)" | sed 's/proto/pb.go/g'`
...
%-image:
	...
	if [ $(DOCKER_PUSH) = true ] && [ $(IMAGE_NAMESPACE) != nholuongut ] ; then docker push $(IMAGE_NAMESPACE)/$*:$(VERSION) ; fi
...
swagger: \
	...
	pkg/apiclient/workflowarchive/workflow-archive.swagger.json \
	pkg/apiclient/workflowtemplate/workflow-template.swagger.json \
	manifests/base/crds/full/nholuongut.io_workflows.yaml \
	manifests \
...
pkg/apis/workflow/v1alpha1/generated.proto: $(GOPATH)/bin/go-to-protobuf $(PROTO_BINARIES) $(TYPES) $(GOPATH)/src/github.com/gogo/protobuf
	...
	$(GOPATH)/bin/go-to-protobuf \
		...
		--packages=github.com/nholuongut/argo-workflows/v3/pkg/apis/workflow/v1alpha1 \
...
manifests/base/crds/full/nholuongut.io_workflows.yaml: $(GOPATH)/bin/controller-gen $(TYPES) ./hack/crdgen.sh ./hack/crds.go
	./hack/crdgen.sh
...
install: githooks
	...
	kubectl kustomize --load-restrictor=LoadRestrictionsNone test/e2e/manifests/$(PROFILE) | sed 's|quay.io/nholuongut/|$(IMAGE_NAMESPACE)/|' | sed 's/namespace: argo/namespace: $(KUBE_NAMESPACE)/' | kubectl -n $(KUBE_NAMESPACE) apply --prune -l app.kubernetes.io/part-of=argo -f -
...
.PHONY: argosay
argosay:
	cd test/e2e/images/argosay/v2 && docker build . -t nholuongut/argosay:v2
ifeq ($(K3D),true)
	k3d image import -c $(K3D_CLUSTER_NAME) nholuongut/argosay:v2
endif
ifeq ($(DOCKER_PUSH),true)
	docker push nholuongut/argosay:v2
endif
...
pull-images:
	...
	docker pull mysql:8
	docker pull nholuongut/argosay:v1
	docker pull nholuongut/argosay:v2
...
logs: $(GOPATH)/bin/stern
	stern -l workflows.nholuongut.io/workflow 2>&1
...
pkg/apis/workflow/v1alpha1/openapi_generated.go: $(GOPATH)/bin/openapi-gen $(TYPES)
	...
	$(GOPATH)/bin/openapi-gen \
	  --go-header-file ./hack/custom-boilerplate.go.txt \
	  --input-dirs github.com/nholuongut/argo-workflows/v3/pkg/apis/workflow/v1alpha1 \
	  --output-package github.com/nholuongut/argo-workflows/v3/pkg/apis/workflow/v1alpha1 \
...
pkg/apis/workflow/v1alpha1/zz_generated.deepcopy.go: $(TYPES)
	...
	bash $(GOPATH)/pkg/mod/k8s.io/code-generator@v0.21.5/generate-groups.sh \
	    ...
	    github.com/nholuongut/argo-workflows/v3/pkg/client github.com/nholuongut/argo-workflows/v3/pkg/apis \
	    workflow:v1alpha1 \
	    --go-header-file ./hack/custom-boilerplate.go.txt
	# Delete the link
	[ -e ./v3 ] && rm -rf v3
