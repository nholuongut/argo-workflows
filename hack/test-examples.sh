...
grep -lR 'workflows.nholuongut.io/test' examples/*  | while read f ; do
  echo " - $f"
  test 0 == $(grep -o 'image: .*' $f | grep -cv 'nholuongut/argosay:v2\|python:alpine3.6')
done

grep -lR 'workflows.nholuongut.io/test' examples/* | while read f ; do
  kubectl delete workflow -l workflows.nholuongut.io/test
  echo "Running $f..."
  kubectl create -f $f
