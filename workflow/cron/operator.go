...
import (
	...
	typed "github.com/nholuongut/argo-workflows/v3/pkg/client/clientset/versioned/typed/workflow/v1alpha1"
...
func newCronWfOperationCtx(cronWorkflow *v1alpha1.CronWorkflow, wfClientset versioned.Interface, metrics *metrics.Metrics) *cronWfOperationCtx {
	return &cronWfOperationCtx{
		...
		wfClient:    wfClientset.nholuongutV1alpha1().Workflows(cronWorkflow.Namespace),
		cronWfIf:    wfClientset.nholuongutV1alpha1().CronWorkflows(cronWorkflow.Namespace),
...
func (woc *cronWfOperationCtx) validateCronWorkflow() error {
	wftmplGetter := templateresolution.WrapWorkflowTemplateInterface(woc.wfClientset.nholuongutV1alpha1().WorkflowTemplates(woc.cronWf.Namespace))
	cwftmplGetter := templateresolution.WrapClusterWorkflowTemplateInterface(woc.wfClientset.nholuongutV1alpha1().ClusterWorkflowTemplates())
