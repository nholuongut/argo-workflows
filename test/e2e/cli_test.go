...
func (s *CLISuite) TestSubmitDryRun() {
	s.Given().
		RunCli([]string{"submit", "smoke/basic.yaml", "--dry-run", "-o", "yaml", "-l", "workflows.nholuongut.io/test=true"}, func(t *testing.T, output string, err error) {
			if assert.NoError(t, err) {
...
func (s *CLISuite) TestSubmitInvalidWf() {
	s.Given().
		RunCli([]string{"submit", "smoke/basic-invalid.yaml", "-l", "workflows.nholuongut.io/test=true"}, func(t *testing.T, output string, err error) {
...
func (s *CLISuite) TestSubmitServerDryRun() {
	s.Given().
		RunCli([]string{"submit", "smoke/basic.yaml", "--server-dry-run", "-o", "yaml", "-l", "workflows.nholuongut.io/test=true"}, func(t *testing.T, output string, err error) {
			if assert.NoError(t, err) {
...
func (s *CLISuite) TestRoot() {
	s.Run("Submit", func() {
		s.Given().RunCli([]string{"submit", "testdata/basic-workflow.yaml", "-l", "workflows.nholuongut.io/test=true"}, func(t *testing.T, output string, err error) {
			if assert.NoError(t, err) {
	...
	s.T().Skip("https://github.com/nholuongut/argo-workflows/issues/7111")
	s.Run("From", func() {
		s.Given().
			CronWorkflow("@cron/basic.yaml").
			When().
			CreateCronWorkflow().
			RunCli([]string{"submit", "--from", "cronworkflow/test-cron-wf-basic", "--scheduled-time", "2006-01-02T15:04:05-07:00", "-l", "workflows.nholuongut.io/test=true"}, func(t *testing.T, output string, err error) {
				assert.NoError(t, err)
			...
			Then().
			ExpectWorkflow(func(t *testing.T, metadata *metav1.ObjectMeta, status *wfv1.WorkflowStatus) {
				assert.Equal(t, "2006-01-02T15:04:05-07:00", metadata.Annotations["workflows.nholuongut.io/scheduled-time"])
			})
	})
}
...
func (s *CLISuite) TestSubmitClusterWorkflowTemplate() {
	s.Given().
		...
		RunCli([]string{"submit", "--from", "clusterworkflowtemplate/cluster-workflow-template-whalesay-template", "--name", "my-wf", "-l", "workflows.nholuongut.io/test=true"}, func(t *testing.T, output string, err error) {
			assert.NoError(t, err)
		}).
		WaitForWorkflow(fixtures.ToBeSucceeded)
}
...
func (s *CLISuite) TestWorkflowDeleteAll() {
	s.Given().
		...
		SubmitWorkflow().
		Given().
		RunCli([]string{"delete", "--all", "-l", "workflows.nholuongut.io/test"}, func(t *testing.T, output string, err error) {
			if assert.NoError(t, err) {
				assert.Regexp(t, "Workflow 'basic-.*' deleted", output)
			}
		})
...
func (s *CLISuite) TestWorkflowDeleteCompleted() {
	s.Given().
		...
		SubmitWorkflow().
		Given().
		RunCli([]string{"delete", "--completed", "-l", "workflows.nholuongut.io/test"}, func(t *testing.T, output string, err error) {
			if assert.NoError(t, err) {
...
func (s *CLISuite) TestWorkflowDeleteResubmitted() {
	s.Given().
		...
		When().
		Given().
		RunCli([]string{"delete", "--resubmitted", "-l", "workflows.nholuongut.io/test"}, func(t *testing.T, output string, err error) {
			if assert.NoError(t, err) {
				assert.Contains(t, output, "deleted")
			}
		})
...
func (s *CLISuite) TestWorkflowDeleteOlder() {
	s.Given().
		...
		WaitForWorkflow().
		Given().
		RunCli([]string{"delete", "--older", "1d", "-l", "workflows.nholuongut.io/test"}, func(t *testing.T, output string, err error) {
			if assert.NoError(t, err) {
				// nothing over a day should be deleted
				assert.NotContains(t, output, "deleted")
			}
		}).
		RunCli([]string{"delete", "--older", "0s", "-l", "workflows.nholuongut.io/test"}, func(t *testing.T, output string, err error) {
			if assert.NoError(t, err) {
				assert.Contains(t, output, "deleted")
			}
		})
...
func (s *CLISuite) TestWorkflowDeleteByPrefix() {
	s.Given().
		...
		SubmitWorkflow().
		Given().
		RunCli([]string{"delete", "--prefix", "missing-prefix", "-l", "workflows.nholuongut.io/test"}, func(t *testing.T, output string, err error) {
			if assert.NoError(t, err) {
				// nothing should be deleted
				assert.NotContains(t, output, "deleted")
			}
		}).
		RunCli([]string{"delete", "--prefix", "basic", "-l", "workflows.nholuongut.io/test"}, func(t *testing.T, output string, err error) {
			if assert.NoError(t, err) {
				assert.Contains(t, output, "deleted")
			}
		})
...
func (s *CLISuite) TestWorkflowStopBySelector() {
	s.Given().
		...
		When().
		SubmitWorkflow().
		RunCli([]string{"stop", "--selector", "workflows.nholuongut.io/test=true"}, func(t *testing.T, output string, err error) {
			if assert.NoError(t, err) {
				assert.Regexp(t, "workflow basic-.* stopped", output)
			}
		})
...
func (s *CLISuite) TestWorkflowTerminateBySelector() {
	s.Given().
		...
		When().
		SubmitWorkflow().
		RunCli([]string{"terminate", "--selector", "workflows.nholuongut.io/test=true"}, func(t *testing.T, output string, err error) {
			if assert.NoError(t, err) {
				assert.Regexp(t, "workflow basic-.* terminated", output)
			}
		})
...
func (s *CLISuite) TestWorkflowResubmitByLabelSelector() {
	s.Given().
		...
		WaitForWorkflow().
		Given().
		RunCli([]string{"resubmit", "--selector", "workflows.nholuongut.io/test=true"}, func(t *testing.T, output string, err error) {
			if assert.NoError(t, err) {
				assert.Contains(t, output, "Name:")
				assert.Contains(t, output, "Namespace:")
				assert.Contains(t, output, "ServiceAccount:")
...
func (s *CLISuite) TestCron() {
	...
	s.Run("Create Schedule Override", func() {
		s.Given().RunCli([]string{"cron", "create", "cron/basic.yaml", "--schedule", "1 2 3 * *", "-l", "workflows.nholuongut.io/test=true"}, func(t *testing.T, output string, err error) {
			if assert.NoError(t, err) {
	...
	s.Run("Create Parameter Override", func() {
		s.Given().RunCli([]string{"cron", "create", "cron/param.yaml", "-p", "message=\"bar test passed\"", "-l", "workflows.nholuongut.io/test=true"}, func(t *testing.T, output string, err error) {
			if assert.NoError(t, err) {
	...
	s.Run("Create Name Override", func() {
		s.Given().RunCli([]string{"cron", "create", "cron/basic.yaml", "--name", "basic-cron-wf-overridden-name", "-l", "workflows.nholuongut.io/test=true"}, func(t *testing.T, output string, err error) {
			if assert.NoError(t, err) {
	...
	s.Run("Create GenerateName Override", func() {
		s.Given().RunCli([]string{"cron", "create", "cron/basic.yaml", "--generate-name", "basic-cron-wf-overridden-generate-name-", "-l", "workflows.nholuongut.io/test=true"}, func(t *testing.T, output string, err error) {
			if assert.NoError(t, err) {
...
func (s *CLISuite) TestWorkflowTemplateRefSubmit() {
	...
	s.Run("CreateWF", func() {
		s.Given().RunCli([]string{"submit", "testdata/workflow-template-ref.yaml", "-l", "workflows.nholuongut.io/test=true"}, func(t *testing.T, output string, err error) {
			if assert.NoError(t, err) {
				assert.Contains(t, output, "Name:")
				assert.Contains(t, output, "Namespace:")
				assert.Contains(t, output, "Created:")
	...
	s.Run("CreateWFWithCWFTRef", func() {
		s.Given().RunCli([]string{"submit", "testdata/cluster-workflow-template-ref.yaml", "-l", "workflows.nholuongut.io/test=true"}, func(t *testing.T, output string, err error) {
			if assert.NoError(t, err) {
				assert.Contains(t, output, "Name:")
				assert.Contains(t, output, "Namespace:")
				assert.Contains(t, output, "Created:")
...
func (s *CLISuite) TestResourceTemplateStopAndTerminate() {
	s.Run("ResourceTemplateStop", func() {
		s.Given().
			...
			When().
			RunCli([]string{"submit", "functional/resource-template.yaml", "-l", "workflows.nholuongut.io/test=true"}, func(t *testing.T, output string, err error) {
	...
	s.Run("ResourceTemplateTerminate", func() {
		s.Given().
			...
			When().
			RunCli([]string{"submit", "functional/resource-template.yaml", "--name", "resource-tmpl-wf-1", "-l", "workflows.nholuongut.io/test=true"}, func(t *testing.T, output string, err error) {
				assert.Contains(t, output, "Pending")
			}).
			WaitForWorkflow(fixtures.ToHaveRunningPod).
...
func (s *CLISuite) TestArchiveLabel() {
	s.Given().
		...
		When().
		CreateWorkflowTemplates().
		RunCli([]string{"submit", "--from", "workflowtemplate/workflow-template-whalesay-template", "-l", "workflows.nholuongut.io/test=true"}, func(t *testing.T, output string, err error) {
			assert.NoError(t, err)
		}).
		WaitForWorkflow().
		Then().
	...
	s.Run("ListKeys", func() {
		s.Given().
			RunCli([]string{"archive", "list-label-keys"}, func(t *testing.T, output string, err error) {
				if assert.NoError(t, err) {
					lines := strings.Split(output, "\n")
					assert.Contains(t, lines, "workflows.nholuongut.io/test")
				}
			})
	})
	s.Run("ListValues", func() {
		s.Given().
			RunCli([]string{"archive", "list-label-values", "-l", "workflows.nholuongut.io/test"}, func(t *testing.T, output string, err error) {
				if assert.NoError(t, err) {
					lines := strings.Split(output, "\n")
