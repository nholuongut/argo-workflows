...
func (g *Given) checkImages(templates []wfv1.Template) {
	...
	allowed := func(image string) bool {
		return strings.Contains(image, "argoexec:") || image == "nholuongut/argosay:v1" || image == "nholuongut/argosay:v2" || discouraged(image)
	}
	for _, t := range templates {
		...
		if container != nil {
			...
			// (⎈ |docker-desktop:argo)➜  ~ time docker run --rm nholuongut/argosay:v2
			// docker run --rm nholuongut/argosay˜:v2  0.21s user 0.10s system 16% cpu 1.912 total
			// docker run --rm nholuongut/argosay:v1  0.17s user 0.08s system 31% cpu 0.784 total
