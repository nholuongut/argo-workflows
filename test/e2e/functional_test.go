...
func (s *FunctionalSuite) TestDeletingPendingPod() {
	s.Given().
		...
		Exec("kubectl", []string{"-n", "argo", "delete", "pod", "-l", "workflows.nholuongut.io/workflow"}, fixtures.OutputRegexp(`pod "pending-.*" deleted`)).
		Wait(3*time.Second). // allow 3s for reconciliation, we'll create a new pod
		Exec("kubectl", []string{"-n", "argo", "get", "pod", "-l", "workflows.nholuongut.io/workflow"}, fixtures.OutputRegexp(`pending-.*Pending`))
}
...
func (s *FunctionalSuite) TestWorkflowRetention() {
	listOptions := metav1.ListOptions{LabelSelector: "workflows.nholuongut.io/phase=Failed"}
	s.Given().
			...
			func(t *testing.T, es []apiv1.Event) {
				for _, e := range es {
					...
					case "WorkflowNodeRunning":
						...
						assert.Equal(t, e.Annotations["workflows.nholuongut.io/node-name"], nodeName)
						assert.Equal(t, e.Annotations["workflows.nholuongut.io/workflow-uid"], string(uid))
						assert.Contains(t, e.Annotations["workflows.nholuongut.io/workflow-name"], "failed-step-event-")
					case "WorkflowRunning":
					case "WorkflowNodeFailed":
						assert.Equal(t, e.InvolvedObject.Kind, "Pod")
						assert.Contains(t, e.Message, "Failed node failed-step-event-")
						assert.Equal(t, e.Annotations["workflows.nholuongut.io/node-type"], "Pod")
						assert.Equal(t, e.Annotations["workflows.nholuongut.io/node-name"], nodeName)
						assert.Contains(t, e.Annotations["workflows.nholuongut.io/workflow-name"], "failed-step-event-")
						assert.Equal(t, e.Annotations["workflows.nholuongut.io/workflow-uid"], string(uid))
					case "WorkflowFailed":
			...
			func(t *testing.T, es []apiv1.Event) {
				for _, e := range es {
					...
					case "WorkflowNodeFailed":
						assert.Contains(t, e.Message, "Failed node failed-step-event-")
						assert.Equal(t, e.Annotations["workflows.nholuongut.io/node-type"], "Pod")
						assert.Contains(t, e.Annotations["workflows.nholuongut.io/node-name"], "failed-step-event-")
					case "WorkflowFailed":
						assert.Contains(t, e.Message, "exit code 1")
					default:
						assert.Fail(t, e.Reason)
			...
			func(t *testing.T, es []apiv1.Event) {
				for _, e := range es {
					...
					case "WorkflowNodeSucceeded":
						...
						assert.Equal(t, "Pod", e.Annotations["workflows.nholuongut.io/node-type"])
						assert.Contains(t, e.Annotations["workflows.nholuongut.io/node-name"], "success-event-")
...
func (s *FunctionalSuite) TestStepLevelMemozie() {
	s.Given().
		Workflow(`apiVersion: nholuongut.io/v1alpha1
kind: Workflow
metadata:
