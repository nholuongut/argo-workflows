...
import (
	...
	"github.com/nholuongut/argo-workflows/v3/server/event/dispatch"
...
func (s *Controller) ReceiveEvent(ctx context.Context, req *eventpkg.EventRequest) (*eventpkg.EventResponse, error) {
	...
	list, err := auth.GetWfClient(ctx).nholuongutV1alpha1().WorkflowEventBindings(req.Namespace).List(ctx, options)
	if err != nil {
		return nil, err
	}
...
func (s *Controller) ListWorkflowEventBindings(ctx context.Context, in *eventpkg.ListWorkflowEventBindingsRequest) (*wfv1.WorkflowEventBindingList, error) {
	...
	return auth.GetWfClient(ctx).nholuongutV1alpha1().WorkflowEventBindings(in.Namespace).List(ctx, listOptions)
}
