...
func TestConvertCronWorkflowToWorkflow(t *testing.T) {
	cronWfString := `apiVersion: nholuongut.io/v1alpha1
kind: CronWorkflow
metadata:
  name: hello-world
spec:
    ...
    templates:
      - name: whalesay
        container:
          ...
          args: ["hello world"]
`
	expectedWf := `apiVersion: nholuongut.io/v1alpha1
kind: Workflow
metadata:
  annotations:
    annotation2: value2
    workflows.nholuongut.io/scheduled-time: "2021-02-19T10:29:05-08:00"
  creationTimestamp: null
  generateName: hello-world-
  labels:
    label1: value1
    workflows.nholuongut.io/cron-workflow: hello-world
  ownerReferences:
...
`
	...
	cronWfInstanceIdString := `apiVersion: nholuongut.io/v1alpha1
kind: CronWorkflow
metadata:
  name: hello-world
  labels:
    workflows.nholuongut.io/controller-instanceid: test-controller
spec:
  schedule: "* * * * *"
  workflowMetadata:
    labels:
...
func TestConvertWorkflowTemplateToWorkflow(t *testing.T) {
	...
	t.Run("ConvertWorkflowFromWFT", func(t *testing.T) {
		...
		assert.NotNil(t, wf)
		assert.Equal(t, "workflow-template-whalesay-template", wf.Labels["workflows.nholuongut.io/workflow-template"])
...
func TestConvertClusterWorkflowTemplateToWorkflow(t *testing.T) {
	...
	assert.NotNil(t, wf)
	assert.Equal(t, "workflow-template-whalesay-template", wf.Labels["workflows.nholuongut.io/cluster-workflow-template"])
	assert.NotNil(t, wf.Spec.WorkflowTemplateRef)
	assert.Equal(t, wfTmpl.Name, wf.Spec.WorkflowTemplateRef.Name)
