...
import (
	...
	"github.com/nholuongut/argo-workflows/v3/pkg/apiclient/workflow"
	workflowmocks "github.com/nholuongut/argo-workflows/v3/pkg/apiclient/workflow/mocks"
...
func Test_listWorkflows(t *testing.T) {
	...
	t.Run("Status", func(t *testing.T) {
		workflows, err := list(&metav1.ListOptions{LabelSelector: "workflows.nholuongut.io/phase in (Pending,Running)"}, listFlags{status: []string{"Running", "Pending"}})
		if assert.NoError(t, err) {
			assert.NotNil(t, workflows)
		}
	})
	t.Run("Completed", func(t *testing.T) {
		workflows, err := list(&metav1.ListOptions{LabelSelector: "workflows.nholuongut.io/completed=true"}, listFlags{completed: true})
		if assert.NoError(t, err) {
			assert.NotNil(t, workflows)
		}
	})
	t.Run("Running", func(t *testing.T) {
		workflows, err := list(&metav1.ListOptions{LabelSelector: "workflows.nholuongut.io/completed!=true"}, listFlags{running: true})
		if assert.NoError(t, err) {
			assert.NotNil(t, workflows)
		}
	})
