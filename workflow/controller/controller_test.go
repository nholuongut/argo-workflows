...
import (
	...
	"github.com/nholuongut/pkg/sync"
	...
	fakewfclientset "github.com/nholuongut/argo-workflows/v3/pkg/client/clientset/versioned/fake"
	"github.com/nholuongut/argo-workflows/v3/pkg/client/clientset/versioned/scheme"
	wfextv "github.com/nholuongut/argo-workflows/v3/pkg/client/informers/externalversions"
	envutil "github.com/nholuongut/argo-workflows/v3/util/env"
	armocks "github.com/nholuongut/argo-workflows/v3/workflow/artifactrepositories/mocks"
...
func newController(options ...interface{}) (context.CancelFunc, *WorkflowController) {
	...
	wfc := &WorkflowController{
		Config: config.Config{
			...
			Images: map[string]config.Image{
				...
				"nholuongut/argosay:v2":    {Command: []string{""}},
	...
	// always compare to WorkflowController.Run to see what this block of code should be doing
	{
		...
		wfc.wfTaskSetInformer = informerFactory.nholuongut().V1alpha1().WorkflowTaskSets()
		wfc.wftmplInformer = informerFactory.nholuongut().V1alpha1().WorkflowTemplates()
		...
		wfc.cwftmplInformer = informerFactory.nholuongut().V1alpha1().ClusterWorkflowTemplates()
...
func expectWorkflow(ctx context.Context, controller *WorkflowController, name string, test func(wf *wfv1.Workflow)) {
	wf, err := controller.wfclientset.nholuongutV1alpha1().Workflows("").Get(ctx, name, metav1.GetOptions{})
	if err != nil {
		panic(err)
	}
	test(wf)
		...
		if node.Type == wfv1.NodeTypePod && node.Phase == wfv1.NodeRunning {
			pod, _ := podcs.Create(ctx, &apiv1.Pod{
				ObjectMeta: metav1.ObjectMeta{
					Name: node.ID,
					Annotations: map[string]string{
						"workflows.nholuongut.io/node-name": node.Name,
					},
					Labels: map[string]string{
						"workflows.nholuongut.io/workflow": woc.wf.Name,
					},
				},
...
func TestIsArchivable(t *testing.T) {
	...
	lblSelector.MatchLabels["workflows.nholuongut.io/archive-strategy"] = "true"
	...
	t.Run("ConfiguredSelector", func(t *testing.T) {
		...
		workflow.Labels["workflows.nholuongut.io/archive-strategy"] = "true"
		assert.True(t, controller.isArchivable(workflow))
	})
}
